...
function getFeaturedApps() {
  ...
  return APPS.filter(a => set.has(a.slug)).slice(0, 12);
}

function FeaturedView() {
  const cards = getFeaturedApps().map(a => `
    <a href="#/app/${a.slug}" class="snap-start shrink-0 w-72">
      <article class="h-full rounded-2xl border border-slate-800 bg-slate-900/60 hover:bg-slate-900 transition shadow-sm hover:shadow-md overflow-hidden">
        <div class="aspect-[16/9] w-full overflow-hidden">
          <img src="${a.cover}" alt="${a.title}" class="w-full h-full object-cover">
        </div>
        <div class="p-4 space-y-2">
          <h3 class="font-semibold leading-tight line-clamp-2">${a.title}</h3>
          <p class="text-xs text-slate-400">${(a.platform || '')} · v${a.version || ''}</p>
          <div class="flex flex-wrap gap-2">${fmtTags((a.tags || []).slice(0,3))}</div>
          <span class="inline-flex items-center gap-1 text-sky-400 text-sm">Ver detalles
            <svg width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="m14 7l-1.4 1.4l2.6 2.6H6v2h9.2l-2.6 2.6L14 17l5-5z"/></svg>
          </span>
        </div>
      </article>
    </a>
  ...
  <section class="mb-8">
    <div class="flex items-center justify-between mb-3">
      <h2 class="text-xl md:text-2xl font-bold">⭐ Destacados del mes</h2>
      <div class="hidden md:flex gap-2">
        <button id="featPrev" class="rounded-xl border border-slate-800 px-3 py-2 text-sm hover:bg-slate-900">←</button>
        <button id="featNext" class="rounded-xl border border-slate-800 px-3 py-2 text-sm hover:bg-slate-900">→</button>
      </div>
    </div>

    <div class="relative">
      <!-- Gradientes de borde -->
      <div class="pointer-events-none absolute inset-y-0 left-0 w-10 bg-gradient-to-r from-slate-950 to-transparent rounded-l-2xl"></div>
      <div class="pointer-events-none absolute inset-y-0 right-0 w-10 bg-gradient-to-l from-slate-950 to-transparent rounded-r-2xl"></div>

      <div id="featuredTrack"
           class="flex gap-4 overflow-x-auto snap-x snap-mandatory px-1 pb-1
                  [scrollbar-width:thin] [&::-webkit-scrollbar]:h-2
                  [&::-webkit-scrollbar-track]:bg-slate-800/60
                  [&::-webkit-scrollbar-thumb]:bg-slate-600/50 [&::-webkit-scrollbar-thumb]:rounded-full">
        ${cards}
      </div>

      <!-- Flechas móviles -->
      <div class="md:hidden mt-3 flex justify-center gap-2">
        <button id="featPrevM" class="rounded-xl border border-slate-800 px-3 py-2 text-sm hover:bg-slate-900">←</button>
        <button id="featNextM" class="rounded-xl border border-slate-800 px-3 py-2 text-sm hover:bg-slate-900">→</button>
      </div>
    </div>
  </section>`;
}
...
function bindFeatured() {
  ...
  if (!track) return;
  const step = () => Math.max(320, track.clientWidth * 0.8);
  const prevBtns = [document.getElementById('featPrev'), document.getElementById('featPrevM')].filter(Boolean);
  const nextBtns = [document.getElementById('featNext'), document.getElementById('featNextM')].filter(Boolean);
  prevBtns.forEach(btn => btn.addEventListener('click', () => track.scrollBy({ left: -step(), behavior: 'smooth' })));
  nextBtns.forEach(btn => btn.addEventListener('click', () => track.scrollBy({ left: step(), behavior: 'smooth' })));
}
